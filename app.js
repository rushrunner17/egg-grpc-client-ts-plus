...
async function getMultiTierServices(clientConfig, app) {
  ...
  if (!(await exists(protoDir))) {
    throw new Error(
      `proto directory not exist: ${protoDir}, app.baseDir = ${app.baseDir}, clientConfig.protoPath = ${clientConfig.protoPath}`
    );
  }
  ...
  for (const protoFile of protoFileList) {
    ...
    const relativeOrAbsolutePath =
      ...
      app.config.grpcClient.loaderOption.includeDirs
        ? protoFile
    ...
    client[methodName] = util.promisify((arg, options, callback) => {
      ...
      method.call(
        ...
        Object.assign({}, options, {
          deadline:
            new Date().getTime() + (Number(clientConfig.timeout) || 10000),
        }),
        (err, res) => {
          if (err) {
